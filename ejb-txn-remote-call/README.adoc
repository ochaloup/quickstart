...
The next step is needed for the directory
link:client/extensions/[extension scripts]
being considered, and the script to be executed from there.
Manually (command `oc edit`) change the created chain build template
to add env variable `S2I_IMAGE_SOURCE_MOUNTS` of the `BuildConfig`.

[source,sh,options="nowrap",subs="+quotes,attributes+"]
----
# Env variable S2I_IMAGE_SOURCE_MOUNTS for BuildConfig -build-artifacts
oc edit template/wildfly-s2i-chained-build-template

# After edit of the yaml template definition, by adding the env variable the result will be like
# (example below contains existing env variables of yaml and the newly added one)
#    sourceStrategy:
#      env:
...
As the final step for the application start the s2i chain build.

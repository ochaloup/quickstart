...
. Install the JDBC driver as https://docs.wildfly.org/22/Developer_Guide.html#Class_Loading_in_WildFly[a jboss module]. Run the command on each server.
+
[source,sh]
----
...
When the setup is done you can start the servers.
Start the `server1` with the `standalone.xml` configuration.
The `server2` and the `server3` comprises a cluster, you need to start them
with the `standalone-ha.xml` configuration.

When started on the same machine the port offset makes
each server to bind at a different port.
...
where `server` application is deployed (ie. __http://localhost:8180/server/commits__
...
. Before the executing the __remote-outbound-fail-stateless__ endpoint do vverify
...
The `XAException.XA_RETRY` exception, meaning an intermittend failure, was reported to the log.
...
For building the application the {productName} s2i funtionality
...
# Manually change the created template to add env variable S2I_IMAGE_SOURCE_MOUNTS
# as environment variable of the BuildConfig -build-artifacts
oc edit template/wildfly-s2i-chained-build-template
# After edit of the yaml by adding the env variable to template the result will be like
#    sourceStrategy:
#      env:
...
# Invoke the s2i chain build for client application
...
# Invoke the s2i chain build for server application
oc new-app --template=wildfly-s2i-chained-build-template \
  -p IMAGE_STREAM_NAMESPACE=$(oc project -q) \
...
The {productName} Operator creates routes that make the applications accessible from
outside of the OpenShift environment. Run the `oc get route` command to find the location of the REST endpoint.
